...
module.exports = {
  ...
  syncCollectionPage: async function (event) {
    ...
    console.log('bulkCollectionInsert', bulkCollectionInsert);
  ...
  syncProducts: async function (event) {
    ...
    if (!_.isNull(event.collectionId)) {
      const collectionDetail = CollectionModel.findById(event.collectionId);
  ...
  syncProductPage: async function (event) {
    ...
    const dbProducts = await ProductModel.where('uniqKey').in(apiProducts.map(product => `${PARTNERS_SHOPIFY}-${product.id}`)).select('_id uniqKey postableByImage');
