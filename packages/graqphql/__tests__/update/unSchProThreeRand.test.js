...
describe('Rule Model', () => {
  ...
  beforeAll(async (done) => {
    ...
    products = await productStub.createProductStub(storeId, 5, collections[0]._id);
    products = await productStub.createProductStub(storeId, 5, collections[1]._id);
    products = await productStub.createProductStub(storeId, 10, collections[2]._id);
  ...
  test(`List Updates`, async () => {
    ...
    const listUpdatesTestCase = {
      ...
      query: `
      query {
        listUpdates(filter: { store: { eq: "${storeId}"}, service: { eq: "${service}"}, scheduleState: [${PENDING}, ${APPROVED}]}, limit: 100) {
          id
          scheduleTime
          scheduleState
          product {
    ...
    expect(result.data.listUpdates.length).toBe(20);
